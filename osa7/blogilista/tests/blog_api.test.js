...
test('a new blog can be created', async () => {
    console.log(userId)
    const newBlog = new Blog({...helper.newBlogEntry, user: userId})
    console.log(newBlog)
    //await api.post('/api/blogs').set('authorization', `bearer ${token}`).send(newBlog)
    await newBlog.save()
    let response = await helper.blogsInDB()
    let exp = newBlog.toJSON()

    expect(response).toContainEqual(exp)
})
...
test('adding a blog returns \"401 Bad Request\" if the request doesn\'t come with a proper token', async () => {
